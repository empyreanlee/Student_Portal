...
public class Confirm2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.confirm2);
        ...
        iconButton1.setOnClickListener(v -> {
            Intent intent = new Intent(Confirm2.this, Confirm.class);
            startActivity(intent);
        });
        iconButton2 = findViewById(R.id.iconButton2);
        iconButton2.setOnClickListener(v -> {
            Intent intent = new Intent(Confirm2.this, Confirm2.class);
            startActivity(intent);
        });
        ...
        buttonClick.setOnClickListener(v -> {
            ...
            if (TextUtils.isEmpty(regNo)) {
                Toast.makeText(Confirm2.this, "Please enter registration number", Toast.LENGTH_SHORT).show();
                return;
            }
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            for (QueryDocumentSnapshot document : task.getResult()) {
                                ...
                                Toast.makeText(Confirm2.this, "Welcome, " + name , Toast.LENGTH_SHORT).show();
                                // Fetch the registered courses
                                fetchRegisteredCourses(document.getId());
                            }
                        } else {
                            // Handle query failure
                            Toast.makeText(Confirm2.this, "Error fetching student details", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
    }
                        ...
                        if (task.isSuccessful()) {
                            ...
                            if (semesterDocument != null && semesterDocument.exists()) {
                                ...
                                if (registeredCourses != null  && !registeredCourses.isEmpty()) {
                                    ...
                                    Toast.makeText(Confirm2.this, "Registered Courses: " + registeredCourses.toString(), Toast.LENGTH_LONG).show();
                                    courseAdapter.updateCourses(registeredCourses);

                                } else {
                                    Log.d("Firestore", "Registered Courses is null or empty");

                                    Toast.makeText(Confirm2.this, "No registered courses found for this semester", Toast.LENGTH_SHORT).show();
                                }
                            } else {
                                Log.d("Firestore", "Semester Document does not exist or is empty");
                                Toast.makeText(Confirm2.this, "Semester document does not exist", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            Toast.makeText(Confirm2.this, "Error fetching registered courses", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
    }
